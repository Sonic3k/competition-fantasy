...
public interface MatchRepository extends JpaRepository<Match, UUID> {
    ...
    @Query("SELECT m FROM Match m WHERE (m.homeTeam.id = :teamId OR m.awayTeam.id = :teamId) AND m.status = 'COMPLETED' ORDER BY m.matchDate DESC NULLS LAST, m.round.roundNumber DESC")
