...
public class MatchController {
    ...
    @GetMapping
    public List<Match> listByRound(@RequestParam(required = false) UUID roundId,
                                    @RequestParam(required = false) UUID seasonId) {
        if (roundId != null) return repo.findByRoundId(roundId);
        if (seasonId != null) return repo.findByRoundSeasonId(seasonId);
        return List.of();
    }

    @GetMapping("/{id}")
    public ResponseEntity<Match> get(@PathVariable UUID id) {
        return repo.findById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<Match> create(@RequestBody Match match, @RequestParam UUID roundId) {
        return roundRepo.findById(roundId).map(r -> {
            match.setRound(r);
            return ResponseEntity.ok(repo.save(match));
        }).orElse(ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}/score")
    @Transactional
    public ResponseEntity<Match> updateScore(@PathVariable UUID id,
